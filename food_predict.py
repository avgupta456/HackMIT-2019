...
def get_food_info(foods):
    ...
    all_food = all_food.astype({3: float, 5: float, 9: float, 15: float, 48: float})
    all_food = all_food.replace([inf, -inf], nan)
    all_food = all_food.fillna(0)

    #cal/g is the amount of calories per gram of food
    all_food["cal/g"] = all_food[3]/all_food[48]
    all_food["fat/g"] = all_food[5]/all_food[48]
    all_food["sugar/g"] = all_food[9]/all_food[48]
    all_food["salt/g"] = all_food[15]/all_food[48]/1000 #mg not g
    all_food["score"] = (all_food["fat/g"])/7.25+(all_food["sugar/g"])/1.75+(all_food["salt/g"])/0.6
    all_food["healthy"] = ["yes" if x<0.5 else "no" for x in all_food["score"]]
